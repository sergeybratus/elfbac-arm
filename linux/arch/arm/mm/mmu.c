...
/*
 * PaX: To avoid #ifdef madness later, contain much of the madness
 * in the below defines
 * Note that not all changes to these mem types are defined in the array
 * below: some of them depend on runtime CPU detection and are applied later.
 */
...
#ifndef CONFIG_ARM_LPAE
	[MT_MINICLEAN_RO] = {
		.prot_sect = PMD_TYPE_SECT | PMD_SECT_XN | PMD_SECT_MINICACHE | PMD_SECT_RDONLY,
		.domain    = DOMAIN_KERNEL,
	},
#endif
	[MT_LOW_VECTORS] = {
...
static void __init build_mem_type_table(void)
{
	...
	if (cpu_arch >= CPU_ARCH_ARMv6 && (cr & CR_XP)) {
		 ...
		 */
		/* PaX: This looks confusing, but remember again that the
		 * PMD_SECT_APX|PMD_SECT_AP_WRITE combination actually make
		 * something read-only
		 */
...
/* PaX: We want to make sure there's no RWX memory in the kernel via
 * virtual alises.  The vectors map is one such target as it's normally
 * allocated at runtime read/write and then mapped to both userland
 * and the kerenl as read/execute.  To solve this, we statically allocate
 * the vectors map and mark it with our __read_only attribute to make it
...
#ifdef CONFIG_PAX_KERNEXEC
		/*
		 * PaX: Upstream does things slightly differently and changes
		 * their code often, we can handle this ourselves correctly
		 */
