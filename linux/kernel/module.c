...
static void unset_module_core_ro_nx(struct module *mod)
{
	set_page_attributes(mod->module_core_rw,
		mod->module_core_rw + mod->core_size,
		set_memory_x);
	...
	for (m = 0; m < ARRAY_SIZE(masks); ++m) {
		for (i = 0; i < info->hdr->e_shnum; ++i) {
			...
			if ((s->sh_flags & masks[m][0]) != masks[m][0]
				...
				continue;
			/*
			 * PaX: record sizes of writable or non-allocatable sections
			 * in core_size_rw, all others in core_size_rx
			 */
...
/* PaX: We redefine the name of this to catch unconverted users */
...
static int move_module(struct module *mod, struct load_info *info)
{
	...
	/*
	 * PaX: Repeats the exact same algorithm as above but against
	 * the RX variant, using pax_open/close_kernel where appropriate
	 * and performing some additional cleanup where needed
	 */
	ptr = module_alloc_update_bounds_rx(mod->core_size_rx);
	kmemleak_not_leak(ptr);
	if (!ptr)
		goto modfree_core_rx;
	...
	for (i = 0; i < info->hdr->e_shnum; i++) {
		...
		/*
		 * PaX: Same basic algorithm as the upstream code, just redirected
		 * to the PaX-specific fields
		 */
...
static int m_show(struct seq_file *m, void *p)
{
	...
	seq_printf(m, "%s %u",
		   mod->name, mod->init_size_rx + mod->init_size_rw +
		   mod->core_size_rx + mod->core_size_rw);
