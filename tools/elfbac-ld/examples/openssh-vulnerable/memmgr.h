...
static inline void* xmemmgr_alloc(ulong nbytes, enum arenas arena_idx)
{
    void *p = memmgr_alloc(nbytes, arena_idx);
    if (!p)
        fatal("xmemmgr_alloc: out of memory (allocating %lu bytes from %u)", nbytes, arena_idx);
    return p;
}
